  ...
  return (
    <section style={{ borderTop: `1px solid ${c.border}`, padding: '72px 20px', background: c.bg }}>
      <div style={{ maxWidth: 980, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 20 }}>
        ...
        <Door c={c} accent={c.amber}
          icon={<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={c.amber} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21h18" /><path d="M5 21V7l8-4v18" /><path d="M19 21V11l-6-4" /><path d="M9 9v.01M9 12v.01M9 15v.01" /></svg>}
          title={t('door1_title')} desc={t('door1_desc')} cta={t('door1_cta')}
          onClick={() => (onResolve ? onResolve() : router.push('/'))} />

        <Door c={c} accent={c.cyan}
          icon={<svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={c.cyan} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" /></svg>}
          title={t('door2_title')} desc={t('door2_desc')} cta={t('door2_cta')}
...
function Door({ c, accent, icon, title, desc, cta, onClick }: {
  c: ReturnType<typeof useLanding>['c']; accent: string; icon: React.ReactNode; title: string; desc: string; cta: string; onClick: () => void
}) {
  return (
